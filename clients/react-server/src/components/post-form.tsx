...
const PostForm = ({ existingPost, isEditMode = false }: PostFormProps): JSX.Element => {
  ...
  const form = useForm<CreatePostRequest>({
    defaultValues: {
      ...
      postNumber: existingPost?.postNumber || 1,
      isPublished: existingPost?.isPublished || false,
      thumbnailUrl: existingPost?.thumbnailUrl || undefined,
      seriesId: existingPost?.seriesId || undefined,
    },
  });

  // Show modal if user has no series when entering the page (only for new posts)
  useEffect(() => {
    if (!isEditMode && (!series || series.length === 0)) {
      setShowNoSeriesModal(true);
    }
  }, [series, isEditMode]);

  // Check for series-id in URL parameters and pre-select the series (only for new posts)
  useEffect(() => {
    if (!isEditMode) {
  ...
  }, [searchParams, series, form, isEditMode]);
